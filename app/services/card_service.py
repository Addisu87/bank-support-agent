...
async def get_card_by_number(db: AsyncSession, card_number: str) -> Card:
    with logfire.span('get_card_by_number', card_number=card_number[-4:]):
        ...
        result = await db.execute(
            select(Card).filter(Card.card_number == card_number)
        )
...
async def create_card(db: AsyncSession, account_id: int, card_data: CardCreate, user_id: int) -> Card:
...
async def update_card(db: AsyncSession, card_id: int, card_data: CardUpdate, user_id: int) -> Card:
...
async def validate_card_transaction(db: AsyncSession, card_number: str, amount: float, merchant: str = None) -> bool:
    """Validate if card can be used for transaction"""
    with logfire.span('validate_card_transaction', card_number=card_number[-4:], amount=amount):
        ...
        if merchant and "international" in merchant.lower() and not card.international_usage:
            return False
...
async def update_card_daily_limit(db: AsyncSession, card_id: int, new_limit: float, user_id: int) -> Card:
...
async def get_card_transactions(db: AsyncSession, card_id: int, user_id: int, limit: int = 10) -> list:
...
async def report_card_lost_or_stolen(db: AsyncSession, card_id: int, user_id: int, reason: str) -> Card:
