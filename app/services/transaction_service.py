...
from app.services.account_service import get_account_by_id, get_account_by_number, update_account_balance
...
async def create_transaction(db: AsyncSession, transaction_data: TransactionCreate) -> Transaction:
    with logfire.span('create_transaction', 
        ...
        transaction = Transaction(
            ...
            reference=transaction_data.reference or await generate_transaction_reference(),
...
async def get_account_transactions(db: AsyncSession, query: TransactionQuery) -> list[Transaction]:
...
async def get_recent_transactions(db: AsyncSession, user_id: int, limit: int = 10) -> list[Transaction]:
...
async def create_transaction_with_card(db: AsyncSession, transaction_data: TransactionCreate, card_number: str) -> Transaction:
    with logfire.span('create_transaction_with_card', 
                     card_number=card_number[-4:],
                     amount=transaction_data.amount):
        # Verify card and account
        if not await validate_card_transaction(db, card_number, transaction_data.amount):
            raise HTTPException(
...
async def get_card_transactions(db: AsyncSession, card_id: int, limit: int = 50, offset: int = 0) -> list[Transaction]:
...
async def create_interbank_transfer(db: AsyncSession, from_account_id: int, to_account_number: str, amount: float, description: str = None) -> dict:
    """Create inter-bank transfer"""
    with logfire.span('interbank_transfer', from_account_id=from_account_id, amount=amount):
        # Get source account
        ...
        if to_account and to_account.bank_id == from_account.bank_id:
            # Same bank transfer
            return await create_same_bank_transfer(db, from_account_id, to_account.id, amount, description)
        else:
            # Inter-bank transfer
            return await create_interbank_transfer_processing(db, from_account, to_account_number, amount, description)

async def create_same_bank_transfer(db: AsyncSession, from_account_id: int, to_account_id: int, amount: float, description: str = None) -> dict:
    """Create same bank transfer"""
    with logfire.span('same_bank_transfer', from_account_id=from_account_id, to_account_id=to_account_id, amount=amount):
        # Update balances
        ...
        transfer_desc = f"Transfer to account {to_account.account_number}" + (f" - {description}" if description else "")
...
async def create_interbank_transfer_processing(db: AsyncSession, from_account: Account, to_account_number: str, amount: float, description: str) -> dict:
    """Process inter-bank transfer (simplified implementation)"""
    with logfire.span('interbank_transfer_processing', from_account=from_account.account_number, amount=amount):
        ...
        transaction = Transaction(
            ...
            amount=-amount,
            transaction_type=TransactionType.TRANSFER,
            description=f"Inter-bank transfer to {to_account_number}" + (f" - {description}" if description else ""),
