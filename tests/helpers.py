...
async def register_user(
    client: AsyncClient, email: str = None, password: str = "testpassword123"
) -> dict:
...
async def create_bank(client: AsyncClient, token: str, bank_data: dict = None) -> dict:
    ...
    response = await client.post(
        "/api/v1/banks/", json=bank_data, headers={"Authorization": f"Bearer {token}"}
    )
    return response
...
async def get_banks(client: AsyncClient, token: str) -> dict:
    ...
    response = await client.get(
        "/api/v1/banks/", headers={"Authorization": f"Bearer {token}"}
    )
    return response


async def create_user_account(
    client: AsyncClient, token: str, account_type: str = "checking"
) -> dict:
    """Create a user bank account and return response"""
    # Get bank ID first
    banks_response = await client.get(
        "/api/v1/banks/", headers={"Authorization": f"Bearer {token}"}
    )
    banks_data = banks_response.json()
    banks_list = (
        banks_data.get("banks", []) if isinstance(banks_data, dict) else banks_data
    )
...
async def create_transaction(
    client: AsyncClient, token: str, account_id: int, transaction_data: dict = None
) -> dict:
...
async def get_transactions(
    client: AsyncClient, token: str, account_id: int = None
) -> dict:
    """Get transactions"""
...
async def create_card(
    client: AsyncClient, token: str, account_id: int, card_data: dict = None
) -> dict:
    """Create a card"""
